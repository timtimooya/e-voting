...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM votesnewmark GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resultnewmark (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM votesnewmark GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
	mysql_query("UPDATE resultnewmark SET count= (SELECT  COUNT( * ) FROM votesnewmark WHERE resultnewmark.v_id=votesnewmark.v_id)") or die (mysql_error()); 
	 
	?>
	<html>
<h1 style="font-size:30;color:#0000FF;">New Mark candidates</h1>
<head>
<style>
table
{
...
$query=mysql_query("SELECT * FROM newmark, resultnewmark WHERE newmark.id = resultnewmark.v_id")or die(mysql_error());
