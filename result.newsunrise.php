...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM votesnewsunrise GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resultnewsunrise (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM votesnewsunrise GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
	 mysql_query("UPDATE resultnewsunrise SET count= (SELECT  COUNT( * ) FROM votesnewsunrise WHERE resultnewsunrise.v_id=votesnewsunrise.v_id)")or die (mysql_error());
	 
	?>
	<html>
<h1 style="font-size:30;color:#0000FF;">New Sunrise candidates</h1>
<head>
<style>
table
{
...
$query=mysql_query("SELECT * FROM newsunrise, resultnewsunrise WHERE newsunrise.id = resultnewsunrise.v_id")or die(mysql_error());
