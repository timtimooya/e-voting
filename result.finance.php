...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM votesfinance_secretary GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resultfinance (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM votesfinance_secretary GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
	mysql_query("UPDATE resultfinance SET count= (SELECT  COUNT( * ) FROM votesfinance_secretary WHERE resultfinance.v_id=votesfinance_secretary.v_id)")or die (mysql_error()); 
	 
	?>
	<html>
<h1 style="font-size:30;color:#0000FF;">finance candidates</h1>
<head>
<style>
table
{
...
$query=mysql_query("SELECT * FROM finance_secretary, resultfinance WHERE finance_secretary.id = resultfinance.v_id")or die(mysql_error());
