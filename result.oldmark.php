...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM votesoldmark GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resultoldmark (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM votesoldmark GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
	 mysql_query("UPDATE resultoldmark SET count= (SELECT  COUNT( * ) FROM votesoldmark WHERE resultoldmark.v_id=votesoldmark.v_id)") or die (mysql_error());
	 
	?>
	<html>
<h1 style="font-size:30;color:#0000FF;">Old mark candidates</h1>
<head>
<style>
table
{
...
$query=mysql_query("SELECT * FROM oldmark, resultoldmark WHERE oldmark.id = resultoldmark.v_id")or die(mysql_error());
