...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM votesequator8_14 GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resulteq8_14 (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM votesequator8_14 GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
	mysql_query("UPDATE resulteq8_14 SET count= (SELECT  COUNT( * ) FROM votesequator8_14 WHERE resulteq8_14.v_id=votesequator8_14.v_id)") or die (mysql_error()); 
	 
	?>
	<html>
<h1 style="font-size:30;color:#0000FF;">equator 8-14 candidates</h1>
<head>
<style>
table
{
...
$query=mysql_query("SELECT * FROM equator8_14, resulteq8_14 WHERE equator8_14.id = resulteq8_14.v_id")or die(mysql_error());
