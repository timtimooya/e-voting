...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM votesvetfarmladies GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resultvetfarmladies (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM votesvetfarmladies GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
	 mysql_query("UPDATE resultvetfarmladies SET count= (SELECT  COUNT( * ) FROM votesvetfarmladies WHERE resultvetfarmladies.v_id=votesvetfarmladies.v_id)")or die (mysql_error());
	 
	?>
	<html>
<h1 style="font-size:30;color:#0000FF;">Vetfarm ladies candidates</h1>
<head>
<style>
table
{
...
$query=mysql_query("SELECT * FROM vetfarmladies, resultvetfarmladies WHERE vetfarmladies.id = resultvetfarmladies.v_id")or die(mysql_error());
