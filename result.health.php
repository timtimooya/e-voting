...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM voteshealth GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resulthealth (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM voteshealth GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
mysql_query("UPDATE resulthealth SET count= (SELECT  COUNT( * ) FROM voteshealth WHERE resulthealth.v_id=voteshealth.v_id)")or die (mysql_error());	 
	 
	?>
	<html>
<h1 style="font-size:30;color:#0000FF;">Health candidates</h1>
<head>
<style>
table
{
...
$query=mysql_query("SELECT * FROM health, resulthealth WHERE health.id = resulthealth.v_id")or die(mysql_error());
