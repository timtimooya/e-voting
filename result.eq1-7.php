...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM votesequator1_7 GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resulteq1_7 (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM votesequator1_7 GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
	 mysql_query("UPDATE resulteq1_7 SET count= (SELECT  COUNT( * ) FROM votesequator1_7 WHERE resulteq1_7.v_id=votesequator1_7.v_id)") or die (mysql_error());
	 
	?>
	<html>
<h1 style="font-size:30;color:#0000FF;">Equator1-7 candidates</h1>
<head>
<style>
table
{
...
$query=mysql_query("SELECT * FROM equator1_7, resulteq1_7 WHERE equator1_7.id = resulteq1_7.v_id")or die(mysql_error());
