...
//$result=mysql_query("SELECT  FROM votespresident where v_id=1,COUNT(*) ") or die (mysql_error()) ;
...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM voteskilimanjaro GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resultkili (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM voteskilimanjaro GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
	 mysql_query("UPDATE resultkili SET count= (SELECT  COUNT( * ) FROM voteskilimanjaro WHERE resultkili.v_id=voteskilimanjaro.v_id)")or die (mysql_error());
	 
	?>
	<html>
<h1 style="font-size:30;color:#0000FF;">kilimanjaro candidates</h1>
<head>
<style>
table
{
...
$query=mysql_query("SELECT * FROM kilimanjaro, resultkili WHERE kilimanjaro.id = resultkili.v_id")or die(mysql_error());
