...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM votesnonresidents GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resultnonresident (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM votesnonresidents GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
	 mysql_query("UPDATE resultnonresident SET count= (SELECT  COUNT( * ) FROM votesnonresidents WHERE resultnonresident.v_id=votesnonresidents.v_id)")or die (mysql_error());
	 
	?>
	<html>
<h1 style="font-size:30;color:#0000FF;">Nonresident candidates</h1>
<head>
<style>
table
{
...
$query=mysql_query("SELECT * FROM nonresident, resultnonresident WHERE nonresident.id = resultnonresident.v_id")or die(mysql_error());
