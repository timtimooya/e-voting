...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM votesinstitute GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resultinstitute (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM votesinstitute GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
	 mysql_query("UPDATE resultinstitute SET count= (SELECT  COUNT( * ) FROM votesinstitute WHERE resultinstitute.v_id=votesinstitute.v_id)")or die (mysql_error());
	 
	?>
	<html>
<h1 style="font-size:30;color:#0000FF;">Institute candidates</h1>
<head>
<style>
table
{
...
$query=mysql_query("SELECT * FROM institute, resultinstitute WHERE institute.id = resultinstitute.v_id")or die(mysql_error());
