...
//LIMIT 0 , 30
	...
	$result=mysql_query("SELECT v_id, COUNT( * ) `Count` FROM votesentertainment GROUP BY v_id") or die (mysql_error());
	//mysql_query("insert into resultpresident1(v_id, count)VALUES('$result')")or die (mysql_error());
	mysql_query("INSERT INTO resultentertainment (v_id, count) SELECT v_id, COUNT( * ) `Count` FROM votesentertainment GROUP BY v_id ON DUPLICATE KEY UPDATE v_id= values(v_id) ") or die (mysql_error());
	mysql_query("UPDATE resultentertainment SET count= (SELECT  COUNT( * ) FROM votesentertainment WHERE resultentertainment.v_id=votesentertainment.v_id)")or die (mysql_error()); 
...
$query=mysql_query("SELECT * FROM entertainment, resultentertainment WHERE entertainment.id = resultentertainment.v_id")or die(mysql_error());
